...
import type { CartItem,CartOperations } from "@/types/cartType"
...
const CartContent = () => {
  ...
  const { data,isFetching } = useProductsFetch();
    ...
    <>
                  ...
                  <tr key={index} className="border-b border-gray-300 ">
                    ...
                    <td className="py-8">
                      <div className="flex items-center">
                        <Button type="button" name="-" class="cursor-pointer hover:opacity-80" 
                        ...
                        <Button type="button" name="+" class={cartItem.quantity == product?.stock 
      ...
      {cart && cart.length > 0 &&
        (<div className="bg-white w-full fixed bottom-0 border-t border-gray-300 flex items-center gap-12 md:px-20 px-8 py-6">
          ...
          <Button type="button" onClick={handleCheckout} name="Proceed to payment" 
      ...
      {cartProductRemove && (
        <Confirmation isOpen={true}
                      ...
                      onCancel={() => setCartProductRemove(undefined)}/>
      )}
